...
export function DashboardNavigation({
  ...
  links: { href: string; label: string; icon: string; group?: string }[];
}) {
  ...
  return (
    <nav className="dashboard-nav-scroll grid max-h-[232px] grid-cols-2 gap-2 overflow-y-auto pr-1 sm:grid-cols-3 lg:max-h-[calc(100vh-138px)] lg:grid-cols-1 lg:gap-1 lg:overflow-y-auto lg:overflow-x-hidden" aria-label="Dashboard navigation">
      {links.map((link) => {
        ...
        const showGroup = Boolean(link.group && link.group !== lastGroup);
        if (link.group) lastGroup = link.group;

        return (
          <div key={link.href} className="contents">
            {showGroup ? (
              <p className="col-span-full mt-2 hidden px-3 pt-1 text-[0.62rem] font-black uppercase tracking-[0.12em] text-[var(--muted)] first:mt-0 lg:block">
                {link.group}
              </p>
            ) : null}
            <Link
