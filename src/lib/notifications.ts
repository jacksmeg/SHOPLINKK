...
export async function notifyBuyerPaymentRequest(input: {
  buyerId: string;
  title: string;
  body: string;
  href?: string;
  ...
  const href = input.href || "/buyer/orders";
  const openUrl = appUrl(href);
  const smsBody = `${input.title}: ${input.body}`.slice(0, 150);
  const amountLine = typeof input.amount === "number" ? `<p><strong>Amount:</strong> GHS ${input.amount.toFixed(2)}</p>` : "";
  ...
  if (user.pushNotificationsEnabled) {
    tasks.push(
      deliverPushToUser(input.buyerId, {
        title: input.title,
        body: input.body,
        href,
        tag: "buyer-payment-request",
      }),
    );
  }
    ...
    tasks.push(
      sendEmail({
        ...
        html: `
          <div style="font-family:Arial,sans-serif;background:#f6f8fc;padding:24px;color:#071b3a">
            <div style="max-width:560px;margin:auto;background:white;border:1px solid #d8e0ef;border-radius:14px;padding:24px">
              <h1 style="font-size:20px;margin:0 0 10px">ShopLinkk payment request</h1>
              <p style="font-size:14px;line-height:1.6">${input.body}</p>
              ${amountLine}
              <p style="font-size:13px;line-height:1.6"><strong>Store:</strong> ${input.storeName || "ShopLinkk seller"}</p>
              <a href="${openUrl}" style="display:inline-block;margin-top:14px;background:#071b3a;color:white;text-decoration:none;border-radius:8px;padding:12px 16px;font-size:13px;font-weight:700">Open buyer orders</a>
            </div>
          </div>
        `,
      }),
