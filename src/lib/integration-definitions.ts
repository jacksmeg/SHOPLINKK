...
export const integrationDefinitions: IntegrationDefinition[] = [
  ...
  },
  {
    ...
    category: "Payments",
    description: "Collect seller listing and advert fees with Hubtel mobile money and card checkout in Ghana.",
    docsUrl: "https://developers.hubtel.com/",
    fields: [
      { key: "apiId", label: "API ID", placeholder: "Hubtel API ID" },
      { key: "apiKey", label: "API key", placeholder: "Hubtel API key", secret: true },
      { key: "merchantAccountNumber", label: "Merchant account number", placeholder: "Hubtel merchant account number" },
      {
        key: "initiateUrl",
        label: "Checkout initiate URL",
        placeholder: "https://payproxyapi.hubtel.com/items/initiate",
        help: "Leave the default unless Hubtel gives you a different live checkout endpoint.",
      },
      {
        key: "statusUrl",
        label: "Transaction status URL",
        placeholder: "https://api-txnstatus.hubtel.com/transactions/{merchantAccountNumber}/status?clientReference={reference}",
        help: "Use {merchantAccountNumber} and {reference}; ShopLinkk replaces them during verification.",
      },
      {
        key: "webhookToken",
        label: "Webhook token",
        placeholder: "Generate a long secret token",
        secret: true,
        help: "Used in the callback URL so only your Hubtel checkout flow can confirm payments.",
      },
    ],
  },
  {
