...
export async function PATCH(
  ...
  const updated = await prisma.foodOrder.update({
    ...
    data: {
      ...
      status: order.status === "PENDING_PAYMENT" ? "PENDING_PAYMENT" : order.status,
  ...
  await notifyUser({
    ...
    type: "SYSTEM",
    title: "Food payment submitted",
    body: `${session.user.name || "A buyer"} submitted direct MoMo payment details for ${order.store.name}.`,
